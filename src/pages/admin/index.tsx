...
export default function AdminDashboard() {
  ...
  function openCreateModal() {
    ...
    setDraft({
      ...EMPTY_DRAFT,
      chapterOrder: String(
        leaves.reduce((max, l) => Math.max(max, l.chapter_order), 0) + 1,
      ),
    });
    setShowModal(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setActionError(
        err instanceof Error ? err.message : "Failed to save this leaf.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete(leaf: Chapter) {
    if (!user) return;
    if (!confirm("Are you sure you wish to strike this leaf from the Tome?")) {
      return;
    }
    ...
    try {
      ...
      setActionError(
        err instanceof Error ? err.message : "Failed to delete this leaf.",
      );
    } finally {
      setPendingId(null);
    }
  ...
  function handleCopyLink(leaf: Chapter) {
    const fullUrl = `${window.location.origin}/u/${leaf.slug}`;
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="rounded-2xl border border-moonlight/20 bg-mist/8 p-6 backdrop-blur-sm">
          ...
          {leavesLoading ? (
            ...
            <p className="py-8 text-center text-sm text-foreground-soft">
              You haven&apos;t authored any leaves yet. Draft your first one
              above.
            </p>
          ) : (
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[0.68rem] font-medium uppercase tracking-[0.3em] text-foreground-soft">
                        {leaf.parent_chapter_id ? "Edited leaf" : "New leaf"} •
                        /{leaf.slug}
                      </span>
                    </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="rounded-xl border border-blood/40 bg-blood/20 px-4 py-2 text-sm font-medium text-foreground hover:bg-blood/30 disabled:opacity-50"
                >
